    ...
    function render($settings=array()) {
        ...
        switch( $this->settings['type']):
            case 'image':
                ...
                echo $this->Form->input( $this->settings['model'].'.'. $this->settings['field'], array('autocomplete'=>'off','class'=>'form-control', 'placeholder' => __('enter code'), 'label' => false, 'required' => true, 'style' => 'width:100px;position:absolute'));
                echo $this->Html->image($this->Html->url(array('controller'=>$controller, 'action'=>$action, '?'=> $qstring), true), array('vspace'=>2, 'style' => 'padding-left:140px;height:32px;'));
                echo $this->Html->link(__('<br><br><i class="glyphicon glyphicon-refresh"></i> '.$this->settings['reload_txt']), '#', array('class' => 'creload', 'escape' => false)); 
